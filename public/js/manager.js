...
window.addDriver = async () => {
  ...
  await addDoc(collection(db, 'users'), {
    nombre: n, apellido: a, password: p, role: 'driver', activo: true, createdAt: serverTimestamp()
  });
...
window.addManager = async () => {
  ...
  if (!n || !a || !p) return showAlert('Completa todos los campos', 'warning');
  await addDoc(collection(db, 'users'), {
    nombre: n, apellido: a, password: p, role: 'manager', activo: true, createdAt: serverTimestamp()
  });
...
function exportHistory() {
  ...
  document.querySelectorAll('#history-list tr').forEach(tr => {
    ...
    if (cells.length >= 4) {
      rows.push({ Chofer: cells[0].textContent, Ruta: cells[1].textContent, Costo: cells[2].textContent, Fecha: cells[3].textContent });
    }
  });
  if (rows.length === 0) return showAlert('No hay datos', 'warning');
...
// ========== BLOQUEO: Solo aviso (NO puede bloquear) ==========
window.blockApp = () => showAlert('⛔ Solo el dueño puede bloquear la app', 'warning');
window.unblockApp = () => showAlert('⛔ Solo el dueño puede desbloquear la app', 'warning');
