...
import { showAlert, exportToExcel, formatDate } from './utils.js';
...
async function loadDrivers() {
  ...
  snap.forEach(d => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <button class="btn btn-sm btn-info" onclick="toggleDriver('${d.id}', ${!data.activo})">${data.activo ? '⏸' : '▶'}</button>
        <button class="btn btn-sm btn-danger" onclick="deleteDriver('${d.id}')">🗑</button>
        <a href="https://wa.me/?text=Hola%20${data.nombre}%20${data.apellido}%2C%20te%20contacta%20el%20dueño" target="_blank" class="btn btn-sm btn-success">📱</a>
      </td>
    `;
    tbody.appendChild(tr);
  });
}
window.toggleDriver = (id, state) => updateDoc(doc(db, 'drivers', id), { activo: state }).then(loadDrivers);
window.deleteDriver = (id) => confirm('¿Borrar chofer?') && deleteDoc(doc(db, 'drivers', id)).then(loadDrivers);
...
async function addDriver() {
  ...
  await addDoc(collection(db, 'drivers'), { nombre: n, apellido: a, password: p, activo: true, createdAt: serverTimestamp() });
...
function listenDriversLocation() {
  const q = query(collection(db, 'drivers'), where('activo', '==', true));
  onSnapshot(q, snap => {
...
async function loadAlerts() {
  ...
  snap.forEach(d => {
    ...
    card.innerHTML = `
      ...
      <br><small>${a.choferNombre} | ${formatDate(a.createdAt)}</small>
...
window.saveAlert = async (id) => {
  ...
  showAlert('Alerta guardada en histórico', 'success');
};
...
window.deleteTrip = (id) => confirm('¿Borrar viaje?') && deleteDoc(doc(db, 'trips', id)).then(loadHistory);

function exportHistory() {
  // Usa exportToExcel de utils.js (requiere SheetJS en HTML)
  showAlert('Implementar con SheetJS - ver utils.js', 'info');
}

// ========== BLOQUEAR APP ==========
async function blockApp() {
  if (!confirm('⚠️ ¿BLOQUEAR la app para TODOS (menos dueño)?')) return;
  await setDoc(doc(db, 'config', 'appStatus'), { blocked: true, blockedBy: sessionStorage.getItem('userId'), at: serverTimestamp() });
...
async function loadStats() {
  ...
  document.getElementById('stat-drivers').textContent = drivers.size;
  document.getElementById('stat-trips').textContent = trips.size;
  document.getElementById('stat-alerts').textContent = alerts.size;
}